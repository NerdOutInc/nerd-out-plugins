...
describe('NodeOAuthClientProvider - OAuth Scope Handling', () => {
  ...
  describe('authorization URL', () => {
    ...
    it('hands the prepared URL to an injected presenter without opening a browser', async () => {
      const authorizationUrlHandler = vi.fn().mockResolvedValue(undefined)
      provider = new NodeOAuthClientProvider({
        ...defaultOptions,
        authorizeResource: 'http://127.0.0.1:38473/mcp',
        authorizationUrlHandler,
      })

      const authUrl = new URL('https://auth.example.com/authorize?state=state-value')
      await provider.redirectToAuthorization(authUrl)

      expect(authorizationUrlHandler).toHaveBeenCalledOnce()
      const handedOff = authorizationUrlHandler.mock.calls[0][0] as URL
      expect(handedOff.searchParams.get('state')).toBe('state-value')
      expect(handedOff.searchParams.get('scope')).toBe('openid email profile')
      expect(handedOff.searchParams.get('resource')).toBe('http://127.0.0.1:38473/mcp')
