...
export function setupOAuthCallbackServerWithLongPoll(options: OAuthCallbackServerOptions) {
  ...
  let authCompletedResolve: (result: { ok: boolean }) => void
  const authCompletedPromise = new Promise<{ ok: boolean }>((resolve) => {
  ...
  app.get('/wait-for-auth', (req, res) => {
    ...
    authCompletedPromise.then(({ ok }) => {
      clearTimeout(longPollTimeout)
      if (!res.headersSent) {
        log(ok ? 'Auth completed during long poll, responding with 200' : 'Auth denied during long poll, responding with 409')
        res.status(ok ? 200 : 409).send(ok ? 'Authentication completed' : 'Authentication denied')
      }
    })
  })
  ...
  app.get(options.path, (req, res) => {
    ...
    if (options.expectedState && state !== options.expectedState) {
      res.status(400).send('Error: Invalid OAuth state')
      return
    }

    const oauthError = typeof req.query.error === 'string' ? req.query.error : undefined
    if (oauthError) {
      const safeError = /^[a-z_]{1,64}$/.test(oauthError) ? oauthError : 'oauth_error'
      authError = new Error(`OAuth authorization failed: ${safeError}`)
      authCompletedResolve({ ok: false })
      options.events.emit('auth-error-received', authError)
      res.status(200).send('Authorization was not granted. You may close this window and return to the application.')
      return
    }
  ...
  const waitForAuthCode = (): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      const onCode = (code: string) => {
        options.events.off('auth-error-received', onError)
        resolve(code)
      }
      ...
      options.events.once('auth-error-received', onError)
    })
  }
