...
describe('setupOAuthCallbackServerWithLongPoll', () => {
  ...
  it('rejects a matching OAuth denial instead of waiting forever', async () => {
    const result = setupOAuthCallbackServerWithLongPoll({
      port: 0,
      path: '/oauth/callback',
      events,
    ...
    const denial = expect(result.waitForAuthCode()).rejects.toThrow('OAuth authorization failed: access_denied')
    const response = await fetch(`http://127.0.0.1:${port}/oauth/callback?error=access_denied&state=expected-state`)

    expect(response.status).toBe(200)
    await denial
  })

  it('does not complete a denial callback whose state mismatches', async () => {
    const result = setupOAuthCallbackServerWithLongPoll({
      port: 0,
      path: '/oauth/callback',
      events,
    ...
    const response = await fetch(`http://127.0.0.1:${port}/oauth/callback?error=access_denied&state=wrong-state`)

    expect(response.status).toBe(400)
    const poll = await fetch(`http://127.0.0.1:${port}/wait-for-auth?poll=false`)
