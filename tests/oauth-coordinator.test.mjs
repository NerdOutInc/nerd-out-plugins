...
function createOAuthFixture({ refreshDelayMs = 0, validAccessTokens = new Set(["access-1"]) } = {}) {
  const counters = {
  ...
  const server = http.createServer(async (request, response) => {
    ...
    if (request.url === "/oauth/register" && request.method === "POST") {
      ...
      response.setHeader("content-type", "application/json");
      response.end(JSON.stringify({ ...metadata, client_id: "phase1-client" }));
      return;
    }
    ...
    if (request.url === "/mcp" && request.method === "GET") {
      ...
      if (!accessToken || !validAccessTokens.has(accessToken)) {
        ...
        response.setHeader(
          "www-authenticate",
          'Bearer resource_metadata="http://127.0.0.1:38474/.well-known/oauth-protected-resource/mcp", scope="notes:read notes:write"'
        );
      } else {
    ...
    if (request.url === "/mcp" && request.method === "POST") {
      ...
      if (message.method === "initialize") counters.authenticatedInitialize += 1;
  ...
  return {
    counters,
    listen: () => new Promise((resolve) => server.listen(38474, "127.0.0.1", resolve)),
...
async function makeHarness() {
  const root = await mkdtemp(path.join(os.tmpdir(), "recall-oauth-coordinator-"));
...
function spawnCoordinator(harness, mode) {
  return spawn(
    process.execPath,
    [coordinator, "--mode", mode, "--client-name", "Claude", "--server-url", serverUrl],
...
async function callbackForRecord(record, query) {
  ...
  for (const [key, value] of Object.entries(query)) callback.searchParams.set(key, value);
...
test("approve, reuse, inspect, denial, state mismatch, and cancel stay browserless and clean", async (t) => {
  ...
  await t.test("approval writes the exact cache and verify-only reuses it", async () => {
    ...
    try {
      ...
      const authorization = await waitFor(
        () => approvalCapture.records.find((record) => record.type === "authorization_required"),
      ...
      assert.deepEqual(await waitForExit(approval), { code: 0, signal: null });
      assert.equal(
        approvalCapture.records.find((record) => record.type === "result")?.status,
        "connected"
      );

      const files = await readdir(harness.cacheDirectory);
      const tokenFile = files.find((file) => file.endsWith("_tokens.json"));
      const clientFile = files.find((file) => file.endsWith("_client_info.json"));
      assert.ok(tokenFile);
      assert.ok(clientFile);
      assert.equal((await stat(path.join(harness.cacheDirectory, tokenFile))).mode & 0o777, 0o600);
      assert.equal((await stat(path.join(harness.cacheDirectory, clientFile))).mode & 0o777, 0o600);
      assert.equal(files.some((file) => file.includes("verifier") || file.includes("lock")), false);
      ...
      assert.equal(verifyCapture.records.some((record) => record.type === "authorization_required"), false);
      assert.equal(
        verifyCapture.records.find((record) => record.type === "result")?.status,
        "connected"
      );
      assert.ok(fixture.counters.requests > requestsBeforeVerify);

      const foreignAuthLock = path.join(harness.cacheDirectory, `${serverHash}_lock.json`);
      await writeFile(
        foreignAuthLock,
        JSON.stringify({ pid: process.pid, port: 49_151, timestamp: Date.now() }),
        { mode: 0o600 }
      );
      const verifyWithForeignLock = spawnCoordinator(harness, "verify-only");
      captureProcess(verifyWithForeignLock);
      assert.deepEqual(await waitForExit(verifyWithForeignLock), { code: 0, signal: null });
      ...
      assert.equal(await readFile(harness.sentinel).then(() => true).catch(() => false), false);

      const captured = `${approvalCapture.stdout()}\n${approvalCapture.stderr()}\n${verifyCapture.stdout()}\n${verifyCapture.stderr()}`;
      assert.equal(["access-1", "refresh-1", "approved-code"].some((secret) => captured.includes(secret)), false);
    } finally {
      await rm(harness.root, { force: true, recursive: true });
    }
  });

  await t.test("denial completes promptly and cleans transient state", async () => {
    const harness = await makeHarness();
    try {
      const child = spawnCoordinator(harness, "authorize");
      const capture = captureProcess(child);
      const authorization = await waitFor(
        () => capture.records.find((record) => record.type === "authorization_required"),
        `coordinator emitted no authorization request: ${capture.stderr()}`
      );
      const state = new URL(authorization.authorizationUrl).searchParams.get("state");
      const response = await callbackForRecord(authorization, { error: "access_denied", state });
      assert.equal(response.status, 200);
      assert.deepEqual(await waitForExit(child), { code: 0, signal: null });
      assert.equal(capture.records.find((record) => record.type === "result")?.status, "denied");
      const files = await readdir(harness.cacheDirectory);
      assert.equal(files.some((file) => file.includes("tokens") || file.includes("verifier") || file.includes("lock")), false);
      assert.equal(await readFile(harness.sentinel).then(() => true).catch(() => false), false);
    } finally {
      await rm(harness.root, { force: true, recursive: true });
    }
  ...
  await t.test("wrong state remains pending until cancellation cleans it", async () => {
    const harness = await makeHarness();
    try {
      const child = spawnCoordinator(harness, "authorize");
      const capture = captureProcess(child);
      ...
      assert.equal(capture.records.some((record) => record.type === "result"), false);
      child.kill("SIGTERM");
      assert.deepEqual(await waitForExit(child), { code: 0, signal: null });
      assert.equal(capture.records.find((record) => record.type === "result")?.status, "cancelled");
      const files = await readdir(harness.cacheDirectory);
      assert.equal(files.some((file) => file.includes("tokens") || file.includes("verifier") || file.includes("lock")), false);
    } finally {
      await rm(harness.root, { force: true, recursive: true });
...
test("verify-only serializes behind a live bridge that is already mid-refresh", async (t) => {
  const fixture = createOAuthFixture({ refreshDelayMs: 600, validAccessTokens: new Set() });
  await fixture.listen();
  t.after(() => fixture.stop());
  const harness = await makeHarness();
  try {
    ...
    const live = spawn(
      process.execPath,
      [proxy, serverUrl, "--allow-http", "--transport", "http-only", "--static-oauth-client-metadata", '{"client_name":"Claude"}'],
      {
        env: { ...harness.environment, MCP_REMOTE_CONFIG_DIR: path.join(harness.root, "recall", "claude") },
    ...
    await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error("live bridge never began refresh")), 10_000)),
    ]);
    const verify = spawnCoordinator(harness, "verify-only");
    const verifyCapture = captureProcess(verify);
    assert.deepEqual(await waitForExit(verify), { code: 0, signal: null });
    assert.equal(verifyCapture.records.at(-1)?.status, "connected");
    await waitFor(() => liveCapture.stdout().includes('"id":1'), `live bridge never recovered: ${liveCapture.stderr()}`);
    live.kill("SIGTERM");
    await waitForExit(live);

    const savedTokens = JSON.parse(
      await readFile(path.join(harness.cacheDirectory, `${serverHash}_tokens.json`), "utf8")
    );
    ...
    assert.equal((await stat(path.join(harness.cacheDirectory, `${serverHash}_tokens.json`))).mode & 0o777, 0o600);
    assert.equal(["access-0", "access-1", "refresh-0", "refresh-1"].some((secret) => captured.includes(secret)), false);
    assert.equal(await readFile(harness.sentinel).then(() => true).catch(() => false), false);
  } finally {
    await rm(harness.root, { force: true, recursive: true });
  }
